...
int bc_not(PARAMETER *sp) {    /* Logisches NOT, INPUT: [float|int] RET: int */
  ...
  if(sp[-1].typ==PL_INT)          sp[-1].integer=~sp[-1].integer;
    ...
    printf("ERROR: incompatible types for NOT.\n");
  }
  return(0);
}
int bc_add(PARAMETER *sp) {    /* binaer addition */
  ...
  if(sp[-1].typ==PL_INT && sp[0].typ==PL_INT)          sp[-1].integer+=sp[0].integer;
    ...
    printf("Inkompatible typen bei Operation ADD.\n");
    free_parameter(sp[0]);
  }
  return(-1);
}
int bc_sub(PARAMETER *sp) {    /* binaer addition */
  ...
  if(sp[-1].typ==PL_INT && sp[0].typ==PL_INT)          sp[-1].integer-=sp[0].integer;
    ...
    printf("Inkompatible typen bei Operation SUB.\n");
    free_parameter(sp[0]);
  }
  return(-1);
}
int bc_mul(PARAMETER *sp) {    /* binaer addition */
  ...
  if(sp[-1].typ==PL_INT && sp[0].typ==PL_INT)          sp[-1].integer*=sp[0].integer;
    ...
    printf("Inkompatible typen bei Operation MUL.\n");
    free_parameter(sp[0]);
  }
  return(-1);
}
int bc_pow(PARAMETER *sp) {    /* binaer addition */
  ...
  if(sp[-1].typ==PL_INT && sp[0].typ==PL_INT)          
   ...
   sp[-1].real=pow(sp[-1].real,(double)sp[0].integer);
    ...
    printf("Inkompatible typen bei Operation POW.\n");
    free_parameter(sp[0]);
  }
    sp[-1].typ=PL_FLOAT;
  return(-1);
...
int bc_div(PARAMETER *sp) {    /* binaer addition */
  ...
  if(sp[-1].typ==PL_INT && sp[0].typ==PL_INT) {
   ...
   sp[-1].typ=PL_FLOAT;
    ...
    printf("Inkompatible typen bei Operation DIV.\n");
    free_parameter(sp[0]);
  }
  return(-1);
}
int bc_mod(PARAMETER *sp) {    /* binaer addition */
  ...
  if(sp[-1].typ==PL_INT && sp[0].typ==PL_INT) {
   ...
   sp[-1].typ=PL_FLOAT;
    ...
    printf("Inkompatible typen bei Operation MOD.\n");
    free_parameter(sp[0]);
  }
  return(-1);
}
int bc_and(PARAMETER *sp) {    /* binaer and */
  ...
  if(sp[-1].typ==PL_INT && sp[0].typ==PL_INT)          sp[-1].integer&=sp[0].integer;
    ...
    printf("Inkompatible typen bei Operation AND.\n");
    free_parameter(sp[0]);
  }
  return(-1);
}
int bc_equal(PARAMETER *sp) {    /* binaer and */
  ...
  if(sp[-1].typ==PL_INT && sp[0].typ==PL_INT)          
    ...
    printf("Inkompatible typen bei Operation EQUAL.\n");
    free_parameter(sp[0]);
  }
  sp[-1].typ=PL_INT;
  return(-1);
...
int bc_greater(PARAMETER *sp) {    /* binaer and */
  ...
  if(sp[-1].typ==PL_INT && sp[0].typ==PL_INT)          
    ...
    printf("Inkompatible typen bei Operation GREATER.\n");
    free_parameter(sp[0]);
  }
  sp[-1].typ=PL_INT;
  return(-1);
...
int bc_less(PARAMETER *sp) {    /* binaer and */
  ...
  if(sp[-1].typ==PL_INT && sp[0].typ==PL_INT)          
    ...
    printf("Inkompatible typen bei Operation LESS.\n");
    free_parameter(sp[0]);
  }
  sp[-1].typ=PL_INT;
  return(-1);
...
int bc_neg(PARAMETER *sp) {    /*  */
  ...
  if(sp[-1].typ==PL_INT)          sp[-1].integer=-sp[-1].integer;
    ...
    printf("Inkompatible typen bei Operation NEG.\n");
  }
  return(0);
}
void cast_to_real(PARAMETER *sp) {
  if(sp->typ==PL_FLOAT) ;
    ...
    printf("ERROR: CAST nicht moeglich/implementiert.\n");
  }
}
...
int bc_sfunc(PARAMETER *sp,int i, int anzarg) {    /*  */
  ...
  printf("ERROR: Hier noch unvollstaendig, diese Funktion bekommt ihre Parameter nicht\n");
  return 1-anzarg;
}
...
int bc_func(PARAMETER *sp,int i, int anzarg) {    /*  */
  ...
  if(pfuncs[i].pmax==1 && (pfuncs[i].opcode&FM_TYP)==F_SQUICK) {
    char test[sp[0].integer+1];
    memcpy(test,sp[0].pointer,sp[0].integer);
    test[sp[0].integer]=0;
    if(pfuncs[i].opcode&F_IRET) {
      sp[0].integer=((int (*)())pfuncs[i].routine)(test);
      sp[0].typ=PL_INT;
    } else {
      sp[0].real=(pfuncs[i].routine)(test);
      sp[0].typ=PL_FLOAT;
    }
    return 1-anzarg;
  }
...
int bc_comm(PARAMETER *sp,int i, int anzarg) {    /*  */
  ...
  printf("ERROR: Hier noch unvollstaendig, diese Procedur bekommt ihre  Parameter nicht\n");
  return -anzarg;
}
...
PARAMETER *virtual_machine(STRING bcpc, int *npar) {
  ...
  while((cmd=bcpc.pointer[i]) && i<bcpc.len) {
    ...
    case BC_PUSHS:
      ...
      i+=a;
      opstack++;
      if(verbose) printf("\"%s\" ",opstack[-1].pointer);
      break;
    case BC_PUSHFUNC:
